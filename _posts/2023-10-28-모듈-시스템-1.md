---
title: "[Javascript] 모듈 시스템(1); CommonJS, ESModules"
date: 2023-10-28
published: true
toc: true
toc_sticky: true
...
React 코드를 작성할 때는 Hooks와 컴포넌트를 가져오기 위해 `import`와 `export`를 사용합니다. 하지만 React 프로젝트의 webpack 설정에서는 `require`과 `module.exports`를 사용하여 다른 모듈을 불러옵니다. 두 방법은 각각 ESModules와 CommonJS 라고 불리는데, 두 방법이 어떤 차이점이 있는지? 또 각각을 어떠한 경우에 사용하는지? 궁금하여 글을 쓰게 되었습니다.
