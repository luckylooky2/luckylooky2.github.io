...
- Next.js가 자동으로 Babel 플러그인(`babel-plugin-styled-components`)을 활성화하여 서버와 클라이언트 className 체크섬의 일치를 보장해요.
...
`Material UI`는 `styled-components`와 호환되지 않는다고 [공식 문서](https://mui.com/material-ui/integrations/styled-components/)에 나와 있어요.
